...
as
SELECT c.UF as UF, COUNT(cl.IDCliente) AS TotalClientes
FROM Cidade c
...
SELECT PD.IDPedido, 
       PD.DataEntrega, 
	   PD.ValorPedido, 
	   COUNT(PIT.IDPedidoItem) AS TOTAL_PRODUTOS, 
	   C.Nome
FROM Pedido PD
INNER JOIN PedidoItem PIT ON PD.IDPedido = PIT.IDPedido
LEFT JOIN Cliente C ON C.IDCliente = PD.IDCliente
WHERE PD.DataEntrega BETWEEN CONVERT(DATETIME,'01/10/2015', 103) AND CONVERT(DATETIME, '31/10/2015', 103)+.99999
...
GROUP BY PD.IDPedido, PD.DataEntrega, PD.ValorPedido, C.Nome
...
SELECT top 1 PN.Nome, COUNT(PN.Nome) AS Ocorrencias
FROM (
