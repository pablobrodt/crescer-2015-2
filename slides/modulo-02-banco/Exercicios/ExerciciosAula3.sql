--Faça uma consulta (query) que retorne apenas o primeiro nome do Associado (tabela associado).
SELECT SUBSTRING(Nome, 0, CHARINDEX(' ', Nome))
FROM Associado;

--Faça uma consulta que retorne o nome dos associados e a idade de cada um (em anos, exibir um número inteiro).
...
--Qual é o associado de maior nome (número de caracteres) ?
SELECT TOP 1 Nome, LEN(Nome)
FROM Associado
ORDER BY LEN(Nome) DESC 


--Faça uma consulta que retorne o nome do associado e a data de quando cada completará 
